...
function uploadImages($uploadedImages,$id_brand,$takeid){
    $files = array();
    $errors = array();
    ...
    $uploadPath = "../image/product/$id_brand";
    if(!is_dir($uploadPath)){
        mkdir($uploadPath);   
    }
    $uploadPathThumb = "../image/thumbnail/product/$id_brand";
    ...
    foreach($files as $file){
        ...
        if ($file != false){
            ...
            $image = 'image/product/'.$id_brand.'/'.$file['name'].'';
            $sql ='insert into product_image(id_product, image) values ('.$takeid.',"'.$image.'")';
            execute($sql);
            //tạo thumbnail
            $thumbnail = "../$image";
            ...
            $thumbnail = 'image/thumbnail/product/'.$id_brand.'/'.$file['name'].'';
            $sql ='insert into product_thumbnail(id_product, thumbnail) values ('.$takeid.',"'.$thumbnail.'")';
            execute($sql);
        }else{
...
$errors = uploadImages($uploadedImages,$id_brand,$takeid);
if(!empty($errors)){
    print_r($errors);
